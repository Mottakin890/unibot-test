...
import { AppView, User } from '../src/types';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, leadCount, knowledgeCount, user, onLogout }) => {
  const navItems = [
    { id: AppView.Chat, label: 'Live Chat', icon: (
    ...
    { id: AppView.Leads, label: 'Contacts & Leads', count: leadCount, icon: (
  ...
  return (
    <div className="md:w-64 bg-slate-900 text-white flex flex-col h-full border-r border-slate-700 flex-shrink-0">
      <div className="p-6 border-b border-slate-800">
        ...
        <p className="text-xs text-slate-400 mt-1">24/7 Intelligent Agent</p>
      </div>
              ...
              >
                ...
                {item.count !== undefined && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    currentView === item.id ? 'bg-blue-500 text-white' : 'bg-slate-700'
                  }`}>
                    {item.count}
                  </span>
                )}
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-800/30">
        ...
        <button 
          onClick={onLogout}
          className="w-full py-2 px-4 rounded-md bg-slate-800 hover:bg-slate-700 text-xs text-slate-300 transition-colors flex items-center justify-center gap-2"
        >
