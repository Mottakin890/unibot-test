...
import { KnowledgeItem, Lead, Chatbot, CustomAction } from "../src/types";
...
`;
  ...
  async *sendMessageStream(
    ...
    if (!keyToUse) {
      yield { text: "Configuration Error: No API Key found." };
      return;
    }
    ...
    try {
      ...
      for await (const chunk of result) {
        ...
        const functionCalls = c.candidates?.[0]?.content?.parts?.[0]?.functionCall 
          ? [c.candidates[0].content.parts[0].functionCall] 
          : c.functionCalls;
